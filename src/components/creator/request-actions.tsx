...
export function RequestActions({ collabId }: { collabId: string }) {
  ...
  return (
    <div className="flex gap-2">
      <button onClick={() => respond('ACCEPTED')} disabled={loading} className="flex-1 h-9 bg-indigo-600 text-white text-xs rounded-lg font-semibold hover:bg-indigo-700 disabled:opacity-50 flex items-center justify-center gap-1.5"><Check className="w-3.5 h-3.5" /> Accept</button>
      <button onClick={() => respond('NEGOTIATING')} disabled={loading} className="h-9 px-3 border border-indigo-200 text-indigo-600 text-xs rounded-lg font-semibold hover:bg-indigo-50 disabled:opacity-50 flex items-center gap-1"><MessageSquare className="w-3.5 h-3.5" /> Negotiate</button>
